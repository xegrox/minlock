...
use wayland_client::{WaylandSource, delegate_noop};
use wayland_client::protocol::{wl_registry, wl_compositor, wl_subcompositor, wl_shm, wl_seat, wl_output, wl_surface};
...
fn main() {
  ...
  let ext_session_lock_mgr: ext_session_lock_manager_v1::ExtSessionLockManagerV1 = globals.bind(&qh, 1..=1, ()).unwrap();
  ...
  let surfaces = globals.contents().clone_list().iter().filter_map(|global| {
  ...
  fn event(
    ...
    match keysym {
      keysyms::KEY_KP_Enter | keysyms::KEY_Return => {
        ...
        state.authenticator.authenticate(state.password.clone(), auth_sender);
      },
...
impl Dispatch<ext_session_lock_surface_v1::ExtSessionLockSurfaceV1, wl_surface::WlSurface> for AppState {
  fn event(
      state: &mut Self,
      ...
      if let ext_session_lock_surface_v1::Event::Configure { serial, width, height } = event {
        ...
        let surface = state.surfaces.iter_mut().find(|surface| surface.as_ref().id() == data.id());
